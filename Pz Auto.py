...
# List of all SFR models
sfr_models = ['Rflat', 'Rm1', 'Rm2', 'Rm3', 'Rh1', 'Rh2', 'Rh3', 'Rf', 'Rf2', 'Rw', 'Rs', 'Rd', 'Rb', 'Rn', 'Rc1', 'Rc2']

# List of all tmin values
tmin_values = [20e-3, 100e-3, 1000e-3, 3000e-3, 5000e-3]

# Loop over all SFR models
for sfr_model in sfr_models:
    # Loop over all tmin values
    for tmin_value in tmin_values:
        # Set the SFR model and tmin value
        sfr_model = sfr_model
        tmin = tmin_value

        directory_path = f"Pz/{sfr_model}"
        if not os.path.exists(directory_path):
            os.makedirs(directory_path)

        # Set the filename
        filename = f"Pz/{sfr_model}/Pz_{sfr_model}_{int(tmin_value * 1e3)}.txt"
